...
fn one_hot_encode(targets: &[usize], num_classes: usize) -> Vec<f32> {
    ...
    for (i, &target) in targets.iter().enumerate() {
        assert!(
            target < num_classes,
            "Target class {} out of bounds (num_classes={})",
            target,
            num_classes
        );
...
mod tests {
    ...
    fn test_cross_entropy_gradients() {
        ...
        assert!(
            grad[0] > 0.0,
            "Gradient at non-target class should be positive"
        );
        assert!(
            grad[1] > 0.0,
            "Gradient at non-target class should be positive"
        ...
        assert!(
            grad_sum_sample0.abs() < 1e-6,
            "Gradients should sum to 0 per sample"
        );
        assert!(
            grad_sum_sample1.abs() < 1e-6,
            "Gradients should sum to 0 per sample"
        );
    ...
    fn test_cross_entropy_numerical_stability() {
        ...
        assert!(
            grad.iter().all(|&g| !g.is_infinite()),
            "Gradients should not be infinite"
        );
